...
use contextual_encoder::{
    ...
    for_rust_char, for_rust_char_checked, for_rust_string, for_sql, for_sql_backslash,
...
fn main() {
    ...
    // unlike the others, this encoder takes exactly one character
    println!("  for_rust_char(\"'\"):           {}", for_rust_char("'"));
    println!(
        "  for_rust_char_checked(input): {:?}",
        for_rust_char_checked(input)
    );
    println!();
