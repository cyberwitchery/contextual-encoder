...
use contextual_encoder::{
    ...
    for_rust_byte_string, for_rust_char, for_rust_string, for_shell_double_quote,
    for_shell_single_quote, for_sql, for_sql_backslash, for_uri_component, for_xml, for_xml11,
...
fn main() {
    ...
    // shell string literals — single-quote (everything literal) or double-quote (escapes specials)
    let shell_input = "$(whoami) && echo '$HOME' is \"$HOME\"";
    println!("--- shell ---");
    println!(
        "  for_shell_single_quote:       {}",
        for_shell_single_quote(shell_input)
    );
    println!(
        "  for_shell_double_quote:       {}",
        for_shell_double_quote(shell_input)
    );
